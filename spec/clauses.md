...
The capability axes draw their concrete values from **namespaces** and a **vocabulary registry**. Both owner-local and community-wide namespaces are supported and coexist; each is published and SAID-addressed so that any verifier can obtain the definitions it needs. In the common case the verifier is the resource owner and already holds its own definitions; a third-party verifier resolves them by SAID.
...
This specification deliberately separates two layers of policy that play opposite roles, and it is important not to read the open layer as a marginal add-on for odd cases. **Open policy is expected to be the principal avenue through which parties express what they actually mean; the closed capability core is the security and safety floor beneath it.** Most real-world authority — "consistent with my stated travel preferences," "only with vendors we have a relationship with," "nothing that would embarrass the firm" — is the stuff of ordinary, interpretable, legally enforceable speech, not of a formal lattice. The closed core exists not to express all policy but to make the parts that bear on security and safety *mechanically provable*, so that no amount of expressive prose above it can breach the floor below.
...
Open policy carried this way is **attributable speech**, and its attribution is structural: it is signed under the issuer's AVID and committed by SAID, so authorship is known and the text is tamper-evident and non-repudiable — exactly the properties that make a written contract clause enforceable. It may be less formally exact than a closed-core bound, but it is interpretable in the way agreements are interpreted in daily life, and it binds to the [Accountability](#accountability) layer on the same footing, with the single accepted cost of possible interpretation gaps.

That cost lands as a *dispute, not a vulnerability*. Because open policy is deny-only, an interpretation difference can never cause an escalation — only a disagreement about whether to deny. Resolution has a clean two-time split:
...
**Guideline — formalize the floor, prose the rest.** Closed core and open policy are not either/or. A delegator SHOULD place anything bearing on security or safety, or anything requiring deterministic agreement between independent verifiers, in the closed core, and use prose only for what genuinely requires judgment. This confines interpretation variance to exactly the judgmental residue — where human-style interpretation is wanted anyway — while keeping the safety-critical part mechanically provable.

> **Non-normative note (formal open policy and existing policy languages).** For the *formal-deterministic* flavor of open policy, established analyzable authorization-policy languages — notably **Cedar** (and similarly Biscuit, OPA/Rego, Datalog) — are RECOMMENDED candidates for the designated policy model: they are decidable, deny-capable, schema-typed, and come with mature tooling and, in Cedar's case, formal verification. Note the layering: such a language is a good fit for the *open* layer and as a *local enforcement / compile target*, but it is **not** a substitute for the closed capability core. A general policy engine evaluates a flat, typically centrally-administered policy set and re-checks attenuation on each exercise; the closed core instead makes attenuation *intrinsic* to a decentralized, cryptographically-chained delegation and supports a soundness theorem (no node exceeds its root). The two belong to different families — centralized ABAC vs. object-capability / certificate-chain — and play complementary roles here.
>
...
### A.7 Open policy preserves soundness

An open policy `O(n)` is a deny-only filter evaluated at exercise. The authorized set is `{ q : q ≤ E(n) ∧ O(n)(q) } ⊆ { q : q ≤ E(n) }`. Since `O(n)` only removes requests, the upper bound of Theorem 1 holds *a fortiori*; no open policy can broaden authority beyond the closed-core result.
